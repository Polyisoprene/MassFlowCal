...
pub trait SnarlViewer<T> {
    ...
    /// Checks if the wire has something to show in context menu if right-clicked.
    #[inline]
    fn has_wire_menu(&mut self, _from: &OutPinId, _to: &InPinId, _snarl: &Snarl<T>) -> bool {
        false
    }

    /// Show context menu for a wire.
    /// This is called when the user right-clicks on a wire.
    #[inline]
    fn show_wire_menu(
        &mut self,
        _from: &OutPin,
    ...
    /// Draws foreground content of the snarl view, on top of wires and nodes.
    ///
    /// `pin_positions` maps OutPinId → output pin position, InPinId → input pin position.
    #[inline]
    fn draw_foreground(
        &mut self,
        _viewport: &Rect,
        ...
        painter: &Painter,
        snarl: &Snarl<T>,
        _out_positions: &HashMap<OutPinId, Pos2>,
        _in_positions: &HashMap<InPinId, Pos2>,
    ) {
        let _ = (painter, snarl);
    }
