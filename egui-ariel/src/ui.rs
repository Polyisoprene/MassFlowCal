    ...
    V: SnarlViewer<T>,
{
    ...
    for (out_pin, in_pin) in all_wires {
        ...
        if !snarl_state.has_new_wires() && snarl_resp.contains_pointer() && hovered_wire.is_none() {
            ...
            if let Some(latest_pos) = latest_pos {
                ...
                if wire_hit {
                    hovered_wire = Some((out_pin, in_pin));

                    let wire_r =
                        ui.interact(snarl_resp.rect, ui.make_persistent_id((out_pin, in_pin)), Sense::click());

                    // Wire context menu (right-click) — use clicked instead of context_menu
                    // to avoid the menu closing when mouse leaves the thin wire
                    if viewer.has_wire_menu(&out_pin, &in_pin, snarl) && wire_r.clicked_by(PointerButton::Secondary) {
                        let op = OutPin::new(snarl, out_pin);
                        let ip = InPin::new(snarl, in_pin);
                        viewer.show_wire_menu(&op, &ip, &mut ui, snarl);
                    } else {
                        // Remove hovered wire by second click (if no custom menu shown)
        ...
        if hovered_wire == Some((out_pin, in_pin)) {
            draw_width *= 1.5;
        }
        ...
        // Render wire widget (label) if enabled
        if viewer.has_wire_widget(&out_pin, &in_pin, snarl) {
            let mid = Pos2::new((from_r.pos.x + to_r.pos.x) / 2.0, (from_r.pos.y + to_r.pos.y) / 2.0);
            let id = ui.make_persistent_id((out_pin, in_pin));
            let (rect, resp) = ui.allocate_exact_size(egui::vec2(1.0, 1.0), egui::Sense::click());
            // Position at wire midpoint using put
            let mut child_ui = ui.new_child(egui::UiBuilder::new().max_rect(egui::Rect::from_center_size(mid, egui::vec2(120.0, 40.0))));
            let op = OutPin::new(snarl, out_pin);
            let ip = InPin::new(snarl, in_pin);
            viewer.show_wire_widget(&op, &ip, &mut child_ui, snarl);
            let _ = (rect, resp, id);
        }
    }

    // Remove hovered wire by second click
    if hovered_wire_disconnect && let Some((w_out, w_in)) = hovered_wire {
        let op = OutPin::new(snarl, w_out);
        let ip = InPin::new(snarl, w_in);
        viewer.disconnect(&op, &ip, snarl);
    }

    // Draw foreground on top of wires and nodes
    let out_positions: HashMap<OutPinId, Pos2> = output_info.iter().map(|(k, v)| (*k, v.pos)).collect();
    let in_positions: HashMap<InPinId, Pos2> = input_info.iter().map(|(k, v)| (*k, v.pos)).collect();
    viewer.draw_foreground(&viewport, &style, ui.style(), ui.painter(), snarl, &out_positions, &in_positions);
